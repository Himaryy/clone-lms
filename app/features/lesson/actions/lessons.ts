...
export async function deleteLessons(id: string) {
...
export async function updateLessonOrders(lessonsIds: string[]) {
  if (lessonsIds.length === 0 || !canUpdateLessons(await getCurrentUser())) {
    return { error: true, message: "Error reordering your lesson" };
  }

  await updateLessonOrdersDb(lessonsIds);

  return { error: false, message: "Successfully reordeing your lesson" };
}
